...
import fsSync from 'fs'; // ✅ Apenas para existsSync
import fs from 'fs/promises'; // ✅ Usar fs assíncrono para evitar bloqueio
...
export const systemStatsRateLimit = rateLimit({
...
let uploadStatsCache: any = null;
let uploadStatsCacheTime: number = 0;
 ...
 */
async function getSafeUploadStats(): Promise<any> {
    const now = Date.now();
    ...
    try {
        ...
        // ✅ Cachear resultado
        uploadStatsCache = result as any;
        uploadStatsCacheTime = now;
 ...
 */
async function getUploadStatsWithTimeout(uploadPath: string): Promise<any> {
...
export const systemHealthRateLimit = rateLimit({
...
export const systemEndpointsRateLimit = rateLimit({
    windowMs: 60 * 1000, // 1 minuto
